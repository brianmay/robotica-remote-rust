...
pub(crate) fn connect(
    ...
    thread::spawn(move || {
        ...
        loop {
            ...
            let button = if let Some(p1) = x.p1 {
                let p1 = translate(p1);
                get_button_for_point(&buttons, p1)
            } else {
                None
            };

            let (do_release, do_press) = match (pressed, button) {
                (None, None) => (None, None),
                (None, Some(button)) => (None, Some(button.id)),
                (Some(button_id), None) => (Some(button_id), None),
                (Some(p), Some(r)) if p == r.id => (None, None),
                (Some(p), Some(r)) => (Some(p), Some(r.id)),
            };
            ...
            pressed = button.map(|button| button.id);
